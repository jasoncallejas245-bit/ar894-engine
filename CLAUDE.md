...
- **Solo moneyline yes/no contracts** (pregame): NFL, NCAAF, MLB, UFC, WNBA, NBA. ATP was dropped 2026-09-09 (too volatile for this strategy). Picks favorites using no-vig fair-probability edges from sportsbook odds vs. Kalshi's own price.
- **Real Kalshi combo trading** (NEW 2026-09-12, see `combo_trading.py`): Kalshi's actual multivariate-event (MVE) combo product, confirmed live to work — bundles multiple moneyline legs into one real combo contract via an RFQ (Request For Quote) negotiation. Off by default (`COMBO_REAL_TRADING_ENABLED=false`); also requires `REAL_TRADING_LEAGUES` non-empty. See "Kalshi's real combo/MVE product" below for the mechanics — it is NOT a normal always-available order.
